...
window.juegosCompletados=juegosCompletados;const resultadoElement=document.createElement("div");resultadoElement.classList.add("resultado-juego");if(btnResetPensar){btnResetPensar.disabled=!0}
if(btnNextPensar){btnNextPensar.disabled=!0}
const puntaje=aciertos;window.puntajesJuegos.pensar=aciertos;resultadoElement.innerHTML=`
      <h2>¡Felicidades!</h2>
...
if(btnResetPensar){btnResetPensar.disabled=!1}
if(btnNextPensar){btnNextPensar.disabled=!1}
seccionJuegoCard.style.display="flex";mostrarSoloSeccionInicio("inicio")})}
function limpiarJuego(){detenerTemporizador();pensarGrid.innerHTML="";const resultadoElement=juegoContainer.querySelector(".resultado-juego");if(resultadoElement){resultadoElement.remove()}
...
const claveJuegosCompletados=`juegosCompletados_${nombreUsuario}`;juegosCompletados=JSON.parse(localStorage.getItem(claveJuegosCompletados))||[];if(juegosCompletados.includes("pensar")){const btnPensar=document.getElementById("iniciar-pensar");if(btnPensar){btnPensar.textContent="FINALIZADO";btnPensar.disabled=!0;btnPensar.classList.add("juego-finalizado")}}
if(juegosCompletados.includes("crear")){const btnCrear=document.getElementById("iniciar-crear");if(btnCrear){btnCrear.textContent="FINALIZADO";btnCrear.disabled=!0;btnCrear.classList.add("juego-finalizado")}}
...
if(juegosCompletados.includes("social")){const btnSocial=document.getElementById("iniciar-social");if(btnSocial){btnSocial.textContent="FINALIZADO";btnSocial.disabled=!0;btnSocial.classList.add("juego-finalizado")}}}else if(juegoContainer.style.display!=="none"&&key==="pensar"){if(!juegoContainer.querySelector(".resultado-juego")){limpiarJuego();iniciarNivel(1);return}}}
if(juegoSocialActivo){if(juegoSocialContainer.style.display!=="none"&&key!=="social"){const hayResultadosVisibles=juegoSocialContainer.querySelector(".social-resultado");if(hayResultadosVisibles&&!juegosCompletados.includes("social")){juegosCompletados.push("social")}
...
if(juegoCrearActivo){if(juegoCrearContainer.style.display!=="none"&&key!=="crear"){const hayResultadosVisibles=juegoCrearContainer.querySelector(".crear-resultado");if(hayResultadosVisibles&&!juegosCompletados.includes("crear")){juegosCompletados.push("crear")}
